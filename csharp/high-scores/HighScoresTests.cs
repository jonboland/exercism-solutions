...
public class HighScoresTests
{
    ...
    [Fact]
    public void List_of_scores_changed()
    {
        var sut = new HighScores(new List<int> { 30, 50, 20, 70 });
        Assert.Equal(new List<int> { 30, 50, 20, 70 }, sut.Scores());
        sut.UpdateScores(new List<int> { 30, 50, 20, 70, 90, 80 });
        Assert.Equal(new List<int> { 30, 50, 20, 70, 90, 80 }, sut.Scores());
    }

    [Fact]
    public void Personal_top_three_from_a_list_of_scores_changed()
    {
        var sut = new HighScores(new List<int> { 10, 30, 90, 30, 100, 20, 10, 0, 30, 40, 40, 70, 70 });
        Assert.Equal(new List<int> { 100, 90, 70 }, sut.PersonalTopThree());
        sut.UpdateScores(new List<int> { 10, 30, 90, 30, 100, 20, 10, 0, 30, 40, 40, 70, 70, 120 });
        Assert.Equal(new List<int> { 120, 100, 90 }, sut.PersonalTopThree());
    }
}